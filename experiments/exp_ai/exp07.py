...
仮説:
- 上場種別はtrainでPR 536件（72.24%）、ST 192件（25.88%）、GR 14件
  （1.89%）と偏っている。fold内で5%未満を「その他」に統合して過学習を抑える。
- 現行all_features_v2へ上場種別だけを追加し、「特徴」は混ぜない。

特徴量・前処理:
- AllFeaturesV2Transformerの20列 + 上場種別。
- 希少カテゴリの判定、中央値補完、One-Hot Encodingは各学習foldだけでfitする。
- 学習foldで全欠損の列だけを除外する。
...
出力:
- experiments/exp_ai/results/exp07/feature_importance.png
- experiments/exp_ai/results/exp07/f1_scores.png
- CSV、JSON、予測値、submissionは出力しない。

実行結果（2026-08-19）:
- 生成特徴量数: 全outer foldで21。
- One-Hot後特徴量数: 36, 35, 37, 37, 36。全欠損除外: なし。
- fold threshold: 0.330, 0.215, 0.255, 0.155, 0.355。
- fold F1: 0.6957, 0.8312, 0.7632, 0.6882, 0.7714。
- fold F1 mean ± std: 0.7499 ± 0.0529。
- nested OOF F1: 0.7481、最終threshold: 0.2620。
- exp01比: -0.0153。希少カテゴリがGRの1種類だけなので統合後も自由度は
  実質変わらず、改善しなかった。
"""
...
SPEC = ExperimentSpec("exp07", "上場種別の希少カテゴリ統合", 7)
