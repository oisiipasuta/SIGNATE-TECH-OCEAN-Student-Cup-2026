...
仮説:
- exp10・exp11で利用されていた第3成分だけを追加すれば、複数成分のノイズを
  避けながら、現行第2成分とは異なるDX展望の軸を利用できる。

特徴量・前処理:
- AllFeaturesV1Transformerの19列 + DX展望SVD第3成分の1列。
- 各学習foldで30成分SVDをfitし、baselineと同じ軸推定条件で第3成分だけを選ぶ。
...
出力:
- experiments/exp_ai/results/exp13/feature_importance.png
- experiments/exp_ai/results/exp13/f1_scores.png
- CSV、JSON、予測値、submissionは出力しない。

実行結果（2026-08-19）:
- 生成特徴量数: 全outer foldで20。
- One-Hot後特徴量数: 33, 32, 34, 34, 33。全欠損除外: なし。
- fold threshold: 0.265, 0.390, 0.315, 0.360, 0.365。
- fold F1: 0.6667, 0.7576, 0.7123, 0.6857, 0.7671。
- fold F1 mean ± std: 0.7179 ± 0.0392。
- nested OOF F1: 0.7158、最終threshold: 0.3390。
- exp01（第2成分のみ）比: -0.0476。第3成分単独も改善しなかった。
"""
...
SPEC = ExperimentSpec("exp13", "DX展望SVD 第3成分のみ", 13)
