"""
実験ID: exp11（Stage 4 の Exp08 相当）
実験名: word unigram（SVD 30・内容語5品詞）
著者: Codex

目的・仮説:
- n-gram の必要性を調べる比較の基準として、word unigram だけを評価する。
- exp12 との差は ngram_range だけに限定する。

特徴量・前処理:
- 入力特徴量は「今後のDX展望」1列。
- MeCab（UniDic）で名詞・動詞・形容詞・形状詞・副詞を抽出する。
- TfidfVectorizer の ngram_range=(1, 1) とし、各学習fold内だけでfitする。
...
出力:
- experiments/exp_dx_outlook_lightgbm/results/exp11/feature_importance.png
- experiments/exp_dx_outlook_lightgbm/results/exp11/f1_scores.png
- CSV、JSON、予測値、submissionは出力しない。

実行結果（2026-08-10実行）:
- 入力特徴量数: 1、TF-IDF語彙数: 4239, 4210, 4193, 4157, 4233
- SVD生成・選択後特徴量数: 各fold 30、除外した全欠損特徴量: なし
- fold threshold: 0.130, 0.185, 0.170, 0.260, 0.315
- fold F1: 0.4793, 0.5000, 0.6000, 0.4854, 0.5500
- fold F1 mean ± std: 0.5230 ± 0.0458
- nested OOF F1: 0.5160、最終threshold: 0.2120
"""
...
EXPERIMENT_ID = "exp11"
EXPERIMENT_NAME = "word unigram（SVD 30・内容語5品詞）"
PARTS_OF_SPEECH = ("名詞", "動詞", "形容詞", "形状詞", "副詞")
NGRAM_RANGE = (1, 1)
FEATURE_LABEL = "word unigram・内容語5品詞・SVD 30"
