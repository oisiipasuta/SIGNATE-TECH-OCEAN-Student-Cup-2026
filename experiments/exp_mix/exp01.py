"""
実験ID: exp01
実験名: all_features_v2 混合特徴量ベースライン
著者: Codex
...
- 入力43列から、all_features_v1 19列と dx_outlook_svd_30_02 の計20列を生成する。
- 数値列は学習foldの中央値で補完する。
...
出力:
- experiments/exp_mix/results/exp01/feature_importance.png
- experiments/exp_mix/results/exp01/f1_scores.png
- CSV、JSON、予測値、submissionは出力しない。

実行結果（2026-08-09実行）:
- 入力列数42、生成特徴量数20、One-Hot後特徴量数24（全outer foldで共通）。
- 除外した全欠損特徴量: なし。
- fold threshold: 0.360, 0.295, 0.265, 0.175, 0.300
- fold F1: 0.7164, 0.8219, 0.7273, 0.6966, 0.7467
- fold F1 mean ± std: 0.7418 ± 0.0432
- nested OOF F1: 0.7402、最終threshold: 0.2790
"""
...
EXPERIMENT_ID = "exp01"
EXPERIMENT_NAME = "all_features_v2 混合特徴量ベースライン"
...
def plot_feature_importance(feature_importance: pd.DataFrame) -> None:
    ...
    ax.set_title("exp01 all_features_v2 LightGBM特徴量重要度", fontsize=16, pad=32)
    ax.text(
        0.5,
        1.01,
...
def plot_f1_scores(result: ExperimentResult) -> None:
    ...
    ax.set_ylabel("F1")
    ax.set_title("exp01 all_features_v2 fold別F1", fontsize=16, pad=32)
    ax.text(
        0.5,
        1.01,
