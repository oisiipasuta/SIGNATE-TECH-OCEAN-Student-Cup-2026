"""
実験ID: exp06
実験名: all_features_v3＋DX変革組織有無
著者: Codex

目的・仮説:
- exp_tree_of_corp/exp08のTop-1構成であるDX変革組織有無だけを
  all_features_v3へ追加し、平均分岐数を除いたときの汎化性能を確認する。
- exp05のPublic LB低下を踏まえ、trainで購入率差が明確だった単一フラグだけを使う。

特徴量・前処理:
- all_features_v3の23列へDX変革組織有無を1列加え、計24列とする。
- AllFeaturesV6Transformerを各fold内でfitする。
...
出力:
- experiments/exp_mix/results/exp06/feature_importance.png
- experiments/exp_mix/results/exp06/f1_scores.png
- 実験スクリプト自体はCSV、JSON、予測値、submissionを出力しない。

実行結果:
- 入力42列から24特徴を生成。One-Hot後37/36/38/38/37列、全欠損除外なし。
- fold閾値: 0.240/0.280/0.160/0.270/0.280、最終閾値=0.2460。
- inner F1: 0.7987/0.7458/0.7321/0.7751/0.7723。
- fold F1: 0.7500/0.8533/0.7442/0.7407/0.8219、平均=0.7820、
  標準偏差=0.0466、nested OOF F1=0.7804。
- exp_tree_of_corp/exp08のTop-1構成を完全再現した。
"""
...
from calc_features.all_features_v6 import (  # noqa: E402
...
EXPERIMENT_ID = "exp06"
EXPERIMENT_NAME = "all_features_v3＋DX変革組織有無"
TARGET_COLUMN = "購入フラグ"
SPEC = ExperimentSpec(EXPERIMENT_ID, EXPERIMENT_NAME, 6)
SELECTED_TREE_FEATURES = ALL_FEATURES_V6_TREE_COLUMNS
...
class FittedFoldModel:
    feature_builder: AllFeaturesV6Transformer
...
def fit_fold_model(X: pd.DataFrame, y: pd.Series) -> FittedFoldModel:
    feature_builder = AllFeaturesV6Transformer(random_state=RANDOM_STATE)
...
def run_nested_cv(X: pd.DataFrame, y: pd.Series) -> ExperimentResult:
    ...
    for fold, (train_index, valid_index) in enumerate(cv.split(X, y)):
        ...
        print(
            f"exp06 fold {fold}: inner F1={inner_f1:.4f}, threshold={threshold:.3f}, "
